...
export class ServerSetRoomService {
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            ...
            `- **All Room**: ห้องทั้งหมดของ MeGuild\n` +
  ...
  public async handleRoomRegistration(@Context() [interaction]: StringSelectContext) {
    ...
    try {
      if (roomType === 'setup_all') {
        this.logger.debug('Setting up all rooms');
        await this.setupAllRooms(interaction);
      } else {
        // ตรวจสอบห้องที่มีอยู่แล้วสำหรับการสร้างห้องทีละประเภท
  ...
  private async setupAllRooms(interaction: StringSelectMenuInteraction<CacheType>) {
    this.logger.debug('setupAllRooms called');

    // ส่งข้อความแจ้งให้ทราบว่าเริ่มสร้าง
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          .setTitle('🏗️ กำลังสร้างห้องทั้งหมด')
          .setDescription(
            '⏳ **กำลังดำเนินการสร้างห้องครบชุด...**\n\n' +
            '📋 รายการห้องที่จะสร้าง:\n' +
            '• Welcome Room (พร้อม User & Eccentric roles)\n' +
            '• News Room\n' +
            '• Register Room (พร้อม Visitor & Adventurer roles)\n' +
            '• Guild Room (พร้อม Guild Head & Co roles)\n' +
            '• Talk Room + 30 Voice Channels\n' +
            '• Busking Room + 5 Voice Channels\n' +
            '• Game Center + GameMatch Room\n\n' +
            '⚠️ **กรุณารอสักครู่** - กระบวนการนี้อาจใช้เวลา 1-2 นาที'
          )
    ...
    // ดึงข้อมูล server เพื่อตรวจสอบห้องที่มีอยู่แล้ว
    const server = await this.serverRepository.getServerById(interaction.guildId);

    try {
      // 1. สร้าง Welcome Room
      try {
        const existingWelcomeChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server[roomFieldMapping.welcome],
        );

        if (existingWelcomeChannel) {
          this.logger.debug('Welcome Room already exists, skipping...');
          skipped.push(`⏭️ Welcome Room (มีอยู่แล้ว: ${existingWelcomeChannel.name})`);
        } else {
          this.logger.debug('Creating Welcome Room...');
          await this.createWelcomeRoom(interaction, defaultRoomNames, roomFieldMapping);
          results.push('✅ Welcome Room');
        }
      } catch (error) {
        this.logger.error('Failed to create Welcome Room:', error);
        errors.push(`❌ Welcome Room: ${error.message}`);
      }

      // 2. สร้าง News Room
      try {
        const existingNewsChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server[roomFieldMapping.news],
        );

        if (existingNewsChannel) {
          this.logger.debug('News Room already exists, skipping...');
          skipped.push(`⏭️ News Room (มีอยู่แล้ว: ${existingNewsChannel.name})`);
        } else {
          this.logger.debug('Creating News Room...');
          await this.createSingleRoom(interaction, 'news', defaultRoomNames, roomFieldMapping);
          results.push('✅ News Room');
        }
      } catch (error) {
        this.logger.error('Failed to create News Room:', error);
        errors.push(`❌ News Room: ${error.message}`);
      }

      // 3. สร้าง Register Room
      try {
        const existingRegisterChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server[roomFieldMapping.register],
        );

        if (existingRegisterChannel) {
          this.logger.debug('Register Room already exists, skipping...');
          skipped.push(`⏭️ Register Room (มีอยู่แล้ว: ${existingRegisterChannel.name})`);
        } else {
          this.logger.debug('Creating Register Room...');
          await this.createRegisterRoom(interaction, defaultRoomNames, roomFieldMapping);
          results.push('✅ Register Room');
        }
      } catch (error) {
        this.logger.error('Failed to create Register Room:', error);
        errors.push(`❌ Register Room: ${error.message}`);
      }

      // 4. สร้าง Guild Room
      try {
        const existingGuildChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server[roomFieldMapping.guild],
        );

        if (existingGuildChannel) {
          this.logger.debug('Guild Room already exists, skipping...');
          skipped.push(`⏭️ Guild Room (มีอยู่แล้ว: ${existingGuildChannel.name})`);
        } else {
          this.logger.debug('Creating Guild Room...');
          await this.createGuildRoom(interaction, defaultRoomNames, roomFieldMapping);
          results.push('✅ Guild Room');
        }
      } catch (error) {
        this.logger.error('Failed to create Guild Room:', error);
        errors.push(`❌ Guild Room: ${error.message}`);
      }

      // 5. สร้าง Talk Room
      try {
        const existingTalkChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server[roomFieldMapping.talk],
        );

        if (existingTalkChannel) {
          this.logger.debug('Talk Room already exists, skipping...');
          skipped.push(`⏭️ Talk Room (มีอยู่แล้ว: ${existingTalkChannel.name})`);
        } else {
          this.logger.debug('Creating Talk Room...');
          await this.createTalkRooms(interaction);
          results.push('✅ Talk Room + 30 Voice Channels');
        }
      } catch (error) {
        this.logger.error('Failed to create Talk Room:', error);
        errors.push(`❌ Talk Room: ${error.message}`);
      }

      // 6. สร้าง Busking Room
      try {
        const existingBuskingChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server.buskingChannel,
        );

        if (existingBuskingChannel) {
          this.logger.debug('Busking Room already exists, skipping...');
          skipped.push(`⏭️ Busking Room (มีอยู่แล้ว: ${existingBuskingChannel.name})`);
        } else {
          this.logger.debug('Creating Busking Room...');
          await this.createBuskingRoom(interaction);
          results.push('✅ Busking Room + 5 Voice Channels');
        }
      } catch (error) {
        this.logger.error('Failed to create Busking Room:', error);
        errors.push(`❌ Busking Room: ${error.message}`);
      }

      // 7. สร้าง Game Center
      try {
        const existingGameChannel = interaction.guild.channels.cache.find(
          (channel) => channel.id === server[roomFieldMapping.gamematch],
        );

        if (existingGameChannel) {
          this.logger.debug('Game Center already exists, skipping...');
          skipped.push(`⏭️ Game Center (มีอยู่แล้ว: ${existingGameChannel.name})`);
        } else {
          this.logger.debug('Creating Game Center...');
          await this.createGameMatchRooms(interaction, defaultRoomNames);
          results.push('✅ Game Center + GameMatch Room');
        }
      } catch (error) {
        this.logger.error('Failed to create Game Center:', error);
        errors.push(`❌ Game Center: ${error.message}`);
      }
      ...
      if (results.length > 0) {
        descriptionParts.push(`**✅ สร้างสำเร็จ ${results.length} รายการ:**\n${results.join('\n')}`);
      }

      if (skipped.length > 0) {
        descriptionParts.push(`**⏭️ ข้ามแล้ว ${skipped.length} รายการ:**\n${skipped.join('\n')}`);
      }

      if (errors.length > 0) {
        descriptionParts.push(`**❌ ล้มเหลว ${errors.length} รายการ:**\n${errors.join('\n')}`);
      }
